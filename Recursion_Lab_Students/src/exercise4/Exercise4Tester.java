...
public class Exercise4Tester {
	...
	public static void main(String[] args) {
		Integer[] arr = {5, 9, 20, 22, 20, 5, 4, 13, 17, 8, 22, 1, 3, 7, 11, 9, 10, 17};
	...
	public static <E> void sort(ArrayList<E> myList){
		if (myList.size() > 1) {     // if size is <= 1, then nothing needs to be done
			Queue<E> q1, q2; 
		    ...
		    int n = myList.size(); 
		    for (int i=0; i<n/2; i++) 
		        q1.enqueue(myList.remove(0));
		    while (!myList.isEmpty()) {
		        q2.enqueue(myList.remove(0)); 
		    }
		    ...
		    while (!q1.isEmpty() && !q2.isEmpty())
		    		if (((Comparable<E>) q1.first()).compareTo(q2.first()) <= 0)
		    			myList.add(q1.dequeue()); 
		    		else 
		    			myList.add(q2.dequeue()); 
